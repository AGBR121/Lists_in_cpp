...
private:
    ...
    class Node{
        ...
        //cambiar la dirección de memoria del nodo siguiente a otro nodo
        void setNext(Node* next){
            next_ = next;
        }

        //cambiar la dirección de memoria del nodo anterior a otro nodo
...
public:
    ...
    void push_front(const T& element){
        ...
        first_->setPrevius(n); //el exprimero ahora tendrá un anterior
        n->setNext(first_); //puntero del siguiente del primer elemento = puntero del exprimer elemento
        first_ = n; //n nuevo first_
        if(size_ == 0){
            last_ = n; //si la lista estaba vacía, last_ tambien apunta a n
        }
        size_++;
    }
    ...
    void push_back(const T& element){
        ...
        if (empty()) {
            ...
            n->setPrevius(last_);//el nuevo ultimo tiene un anterior, que es el exultimo
    ...
    void pop_back(){
        asser(!empty()); //si esta vacío, error
        if(first_ == last_){ //si first y last apuntan a lo mismo
            ...
            Node* temp = last_->getPrevius(); //obtenemos el penultimo nodo
            temp->setNext(nullptr); //cambiamos la direccion del nodo siguiente del penultimo a nulo
            delete last_; //eliminamos el ultimo
            last_ = temp; //el nuevo ultimo es el penultimo
